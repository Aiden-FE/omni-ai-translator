...
| `feature:fullpage:segmenter-pool` | [fullpage-segmenter-pool.md](fullpage-segmenter-pool.md) | 全文翻译分段收集器、并发翻译池与双模式渲染器 - SegmentRecord 契约、collectSegments/runPool/retrySegments、isSegmentInViewport/createViewportObserver 视口工具、applyReplace/applyBilingual/switchMode/restoreAll、Shadow DOM 隔离与自足样式、data-llm-translator 排除约定 |
| `feature:fullpage:orchestrator` | [fullpage-orchestrator.md](fullpage-orchestrator.md) | 全文翻译编排器状态机与可复用模式 - 唯一状态持有者组合无状态组件、startInFlight 并发重入守卫、增量翻译防抖管线（isFlushing+data-llm-translator 过滤+recordedEls 去重）、isActive/onSettled 防闪回双保险、Shadow DOM+自足样式隔离跨模块复用、isBackgroundCommand 类型守卫、审查验证（REVIEW.md 309 单测+15 e2e 全绿） |
